...
const AdminWeekSet = () => {
  ...
  const [saStartDay, setSaStartDay] = useState()
  const [saEndday, setSaEndDay] = useState()
  ...
  async function getWeekData() {
    ...
    var res = await fetch(`${baseUrl}/get-week-set-data-admin`, {
      method: 'GET',
      headers: {
    ...
    if (data.result.code == "200") {
      var totaldata = data.result.list;
      ...
      setStatus(0);
      if (data.result.list[0].week_start == "Sun") {
        setStartDay("Sun-1")
      }
      else if (data.result.list[0].week_start == "Mon") {
        setStartDay("Mon-2")
      }
      else if (data.result.list[0].week_start == "Tue") {
        setStartDay("Tue-3")
      }
      else if (data.result.list[0].week_start == "Wed") {
        setStartDay("Wed-4")
      }
      else if (data.result.list[0].week_start == "Thu") {
        setStartDay("Thu-5")
      }
      else if (data.result.list[0].week_start == "Fri") {
        setStartDay("Fri-6")
      }
      else {
        setStartDay("Sat-7")
      ...
      //----------------------------------//
      if (data.result.list[0].week_end == "Sun") {
        setEndDay("Sun-1")
      }
      else if (data.result.list[0].week_end == "Mon") {
        setEndDay("Mon-2")
      }
      else if (data.result.list[0].week_end == "Tue") {
        setEndDay("Tue-3")
      }
      else if (data.result.list[0].week_end == "Wed") {
        setEndDay("Wed-4")
      }
      else if (data.result.list[0].week_end == "Thu") {
        setEndDay("Thu-5")
      }
      else if (data.result.list[0].week_end == "Fri") {
        setEndDay("Fri-6")
      }
      else {
        setEndDay("Sat-7")
  ...
  async function submitFun(event) {
    ...
    dispatch(getLodderStatus(true))
    await axios.post(`${baseUrl}/add-or-edit-week-set-data-admin`, fd, {
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
      }
    }).then((res) => {
  ...
  async function superAdminApi() {
    ...
    // console.log(data)
    if (data.result.code == "200") {
      setSaStartDay(data.result.list.week_start)
      setSaEndDay(data.result.list.week_end)
    }
  }
  ...
  return (
    <>
      <div className='px-8'>
        ...
        {
          status === 1 ? (<>
          <p>By default week start day is <b>{saStartDay}</b> and end day <b>{saEndday}</b></p>
          </>) : (<>
          <p>By default week start day is <b>{saStartDay}</b> and end day <b>{saEndday}</b> has been overwrite by admin start day and end day</p>
          </>)
        }
