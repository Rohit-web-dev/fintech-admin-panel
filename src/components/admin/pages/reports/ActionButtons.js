...
import { MASTER_REPORT_LIST_CLONE } from '../../../../redux/type';
import { CHILD_REPORT_WITH_TRANSACTION_LIST_CLONE } from '../../../../redux/type';
...
function ActionButtons(props) {
  ...
  async function onItemClick(e) {
    ...
    if (e.itemData == "Aproved") {
      ...
      if (pageType == "parent") {
        var urlEndPoint = `${baseUrl}/master-report-status-update`;
      }
      else if (pageType == "child") {
        var urlEndPoint = `${baseUrl}/child-report-transaction-status-update`;
      } else {
      ...
      await axios.post(`${urlEndPoint}`, fd, {
        ...
        if (res.data.result.code == "200") {
          ...
          dispatch({ type: MASTER_REPORT_LIST_CLONE });
          dispatch({ type: CHILD_REPORT_WITH_TRANSACTION_LIST_CLONE });
        } else {
      ...
      navigate("/report/transaction-detail/" + props.data.data.id)
