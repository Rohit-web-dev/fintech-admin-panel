...
const UpdateExpType = () => {
  ...
  const expId = useSelector(state => state.expTypeIdSlice.exp_id);
  ...
  async function getExpData() {
    ...
    return axios.post(`${baseUrl}/exp-type-view`, fd, {
      ...
      if (res.data.result.code == "200") {
        ...
        setDeptId(res.data.result.expenceDetails.dept_details.id)
        if (res.data.result.dynamic_msg == "false") {
          SetDynamicData([{ key_field: '', type: 'text', 'required_type': 'optional' }])
        } else {
  ...
  const addfun = () => {
    SetDynamicData([...dynmicdata, { key_field: '', type: 'text' , 'required_type': 'optional', }]);
  }
  ...
  const removefun = (index) => {
    const value = [...dynmicdata];
    // console.log(dynmicdata.length,index,value[index])
    if (dynmicdata.length == 1) {
      value[0]['key_field']="";
      value[0]['type']="text";
      value[0]['required_type']="optional";
      SetDynamicData(value);
    } else {
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <h1 className="modal-heading">Edit Expense Type/ {expId}</h1>
      </div>
              ...
              <div className="sm:items-start">
                ...
                <div className="mt-1 sm:mt-0 sm:col-span-2">
                  ...
                  >
                    ...
                    {
                      depts.map((dept) => (
                        <option value={dept.id} key={dept.id} selected={dept.id == deptId ? 'selected' : ''}>{dept.identifier}</option>
                      ))
                    }
                  </select>
                </div>
              ...
              {dynmicdata.map((data, index) => (
                ...
                <div key={index} className="sm:flex sm:gap-3 mt-5 sm:items-center justify-evenly">
                  <div>
                    ...
                    <input type="text" name="key_field" value={data.key_field} onChange={event => dynamicChangeInput(index, event)} placeholder="field"
                  ...
                  <div className="sm:items-start">
                    ...
                    <div className="mt-1 sm:mt-0 sm:col-span-2">
                      ...
                      >
                        <option value={'text'} key={'text'} selected={data.type == 'text' ? 'selected' : ''}> Text</option>
                        <option value={'integer'} key={'integer'} selected={data.type == 'integer' ? 'selected' : ''}> Integer</option>
                        <option value={'float'} key={'float'} selected={data.type == 'float' ? 'selected' : ''} >Float</option>
                        <option value={'number'} key={'number'} selected={data.type == 'number' ? 'selected' : ''} >Number</option>
                        <option value={'tiny-text'} key={'tiny-text'} selected={data.type == 'tiny-text' ? 'selected' : ''} >Tiny-text</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="sm:items-start">
                    ...
                    <div className="mt-1 sm:mt-0 sm:col-span-2">
                      <select
                        ...
                        className="dynamic-select-box p-2 block w-full shadow-sm border border-gray-200 sm:max-w-xs sm:text-sm border-gray-300"
                      >
                        <option value={'optional'} key={'optional'} selected={data.required_type == 'optional' ? 'selected' : ''} >Optional</option>
                        <option value={'required'} key={'required'} selected={data.required_type == 'required' ? 'selected' : ''} >Required</option>
                      </select>
                    </div>
                  </div>
