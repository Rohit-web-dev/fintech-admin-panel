...
const UpdateAdmin = () => {
  ...
  //admin id
  const AdminId = useSelector(state => state.AdminUpdateSlice.admin_id)
  ...
  async function AdminView() {
    ...
    await axios.post(`${baseUrl}/admin-view`, fd, {
      headers: {
      ...
      if (res.data.result.code == "200") {
        ...
        setName(res.data.result.adminDetails.name)
        setEmail(res.data.result.adminDetails.email)

        SetAdminDeptIds(res.data.result.adminDepts)
        SetDeptIds(res.data.result.adminDepts)

        SetAllDept(res.data.result.availableDepts)
      }
      else {
  ...
  function onSelect(selectedList) {
    SetDeptIds(selectedList)
     console.log(deptIds,selectedList)
  ...
  const handleSubmit = async (event) => {
    ...
    //password validation for min 6 characters
    ...
    // console.log(Object.fromEntries(fd), deptIds.length);
    // return false;

    dispatch(getLodderStatus(true))
    await axios.post(`${baseUrl}/admin-update`, fd, {
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
      }
    }).then((res) => {
                ...
                <div className="space-y-6 sm:space-y-5">
                  ...
                  <div className="sm:grid sm:grid-cols-2 sm:gap-4">
                    <div className="sm:items-start">
                      ...
                      <label id="passError" style={{"display":"none"}}>Please enter atleast 6 character of password</label>
                    </div>
