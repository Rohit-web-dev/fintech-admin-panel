...
function ExpensePolicy() {
  ...
  //for dynamic form
  // const [dynmicdata, SetDynamicData] = useState(props.dataone);

  // const dynmicdata = useSelector(state => state.ExpencePolisyListSlice.list);
  // const [dynmicdata, SetDynamicData] = useState([]);
  ...
  //const [totalIndexforAdd, SetTotalIndexforAdd] = useState('');
  const [singleAmount, SetsingleAmount] = useState(0);

  //for getting rem fund and available level under a dept and exp
  const [singedept, SetSingleDept] = useState('');
  ...
  useEffect(() => {
    get_dept();
    get_all_available_expence_type_and_levels();
    getAllExpencePolicy();
    setTimeout(() => {
      // $(".hidden-input-exp").hide();
    }, 1000);
  ...
  async function get_dept() {
    await axios.get(`${baseUrl}/expence-policy-dept-list`, {
      headers: {
  ...
  async function get_all_available_expence_type_and_levels() {  //for get all added values to be selected in select box
    var res = await fetch(`${baseUrl}/expence-policy-all-available-exp-list-and-levels`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
      }
  ...
  async function DeptOnchageFunction(dept_id, index, type) {
    setdeptName("");
    const value = [...arr2];
    ...
    if (dept_id != "") {
      ...
      var res = await fetch(`${baseUrl}/expence-policy-exp-type-under-a-dept`, {
        method: 'POST',
        body: fd,
        headers: {
          'Authorization': `Bearer ` + localStorage.getItem('token')
      ...
      SetFilterExp(data.result.exp_types);
      if (type == "notbutton") {
        value[index]['level_name'] = "";
        value[index]['exp_type_id'] = "";
      }
      SetSingleIndex(index);
      $("#exp_type_id" + index).prop('disabled', false);
      $("#level_name" + index).prop('disabled', false);
    } else {
      toast.warning("select a dept.")
    }
  }
  ...
  //-------Expence type onchange function For Add---------//
  async function getFundAndLevel(exp_id) {

    //console.log(singedept, exp_id, "For add");
    var fd = new FormData;
    fd.append("dept_id", singedept)
    fd.append("exp_type_id", exp_id)
    var res = await fetch(`${baseUrl}/expence-policy-fund-and-levels`, {
      method: 'POST',
      body: fd,
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
    ...
    if (data.result.code == "200") {
      SetFilterlevel(data.result.filter_levels)
      setTotal(data.result.totalFunds)
      setInvested(data.result.investedFunds)
      setRem(data.result.rem)
      // setTotalUsers(data.result.totalUserCount)
      setdeptName(data.result.deptName)
      setexpName(data.result.expName)
    } else {
      console.log(data)
    }
  }
  ...
  //----CLONE FOR EDIT OnExpecneType change-------//
  async function getFundAndLevelClone(dept_id, exp_id, level_name) {
    // alert(2)
    // console.log(dept_id, exp_id, level_name, "For Edit");
    var fd = new FormData;
    fd.append("dept_id", dept_id)
    fd.append("exp_type_id", exp_id)
    fd.append("level_name", level_name)
    var res = await fetch(`${baseUrl}/expence-policy-fund-and-levels-for-edit`, {
      method: 'POST',
      body: fd,
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
    ...
    console.log("j", data);
    if (data.result.code == "200") {
      SetFilterlevel(data.result.all_level_under_dept)
      setTotal(data.result.totalFunds)
      setInvested(data.result.investedFunds)
      setRem(data.result.rem)
      setTotalUsers(data.result.totalUserCount)
  ...
  //Onchange the level dropdown for add and edit
  async function getAllUserUnderADeptAndLevel(singedept, level) {
    var fd = new FormData;
    fd.append("dept_id", singedept)
    fd.append("level_name", level)
    var res = await fetch(`${baseUrl}/expence-policy-total-users`, {
      method: 'POST',
      body: fd,
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
    ...
    if (data.result.code == "200") {
      ...
      //SetTotalIndexforAdd(data.result.totalUser)
  ...
  const dynamicChangeInput = (index, event) => {
    ...
    // if (value[index]['to_date'] != "") {
    //   if (value[index]['from_date'] > value[index]['to_date']) {
    //     alert("start date can not be more that end date")
    //   }
    // }
    setArr2(value);
  }
  ...
  //   //  console.log(AfterIndex.length)
  //   var thisIndex = index + 1;
  //   //  console.log(index,thisIndex)


  //   SetDynamicData([...beforeIndex, { unique_id: '', dept_id: '', exp_type_id: '', from_date: '', to_date: '', level_name: '', amount_per_user: '', amount_per_transaction: '', periodicity: '' }, ...AfterIndex]);
  ...
  //     $("#level_name" + index).prop('disabled', true);
  //     $("#from_date" + thisIndex).prop('disabled', false);
  //     $("#to_date" + thisIndex).prop('disabled', false);
  //     $("#amount_per_user" + thisIndex).prop('disabled', false);
  //     $("#amount_per_transaction" + thisIndex).prop('disabled', false);
  ...
  function addfunnew() {
    ...
    SetsingleAmount(0)
    setTotalUsers('');
    setArr2([{ unique_id: '', dept_id: '', exp_type_id: '', from_date: '', to_date: '', level_name: '', amount_per_user: '', amount_per_transaction: '', periodicity: '' }, ...arr2])
    let thisIndex = 0;
  ...
  const removefun = async (index) => {
    ...
    var res = await fetch(`${baseUrl}/expence-policy-delete`, {
      method: 'POST',
      body: fd,
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
  ...
  // ------------ HIDE AND SHOW BUTTON CLICK------------//
  // [ Description : after clicking the show icon, automatic dept, funds will show. for that we have called onchangeDept function
  //  and onChangeExpenceType function respectively... For onChangeExp we need to call a clone api ]

  const Showfun = (index) => {
    console.log(index)
    // $(".hidden-input-exp" + index).show();

    const value = [...arr2];
    //console.log(value[index]['dept_id'])
    //console.log( $("#btnToggle"+index).html() );
    if ($("#btnToggle" + index).html() == `<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"></path></svg>`) {
      var type = "button";
      SetSingleDept(value[index]['dept_id'])  //making the dept_id fill
      DeptOnchageFunction(value[index]['dept_id'], index, type) //api call onchange dept
      getFundAndLevelClone(value[index]['dept_id'], value[index]['exp_type_id'], value[index]['level_name'])//api call onchange exp
      SetsingleAmount(value[index]['amount_per_user'])
      ...
      $("#amount_per_user" + index).prop('disabled', false);
      $("#amount_per_transaction" + index).prop('disabled', false);
  ...
  async function singleInsertOrUpdateFunction(alldata, type, index) {
    console.log(alldata, type, index + 1); //index+1 for backend index calculation
    // return false
    var fd = new FormData;
    if (type == "edit") {
    ...
    fd.append("level_name", alldata.level_name)
    fd.append("amount_per_user", alldata.amount_per_user)
    fd.append("amount_per_transaction", alldata.amount_per_transaction)
    ...
    dispatch(getLodderStatus(true))
    await axios.post(`${baseUrl}/expence-policy-add-or-edit`, fd, {
      headers: {
        'Authorization': `Bearer ` + localStorage.getItem('token')
      }
    }).then((res) => {
      ...
      if (res.data.result.code == "200") {
        ...
        $("#amount_per_user" + index).prop('disabled', true);
        $("#amount_per_transaction" + index).prop('disabled', true);
  ...
  function showList(index) {
    // $('#list-grid-exp')
    console.log($('#list-grid-exp' + index).text())
    if ($('#list-grid-exp'+index).text() === "User List") {
      SetListStatus(!listStatus)
      $('#list-grid-exp'+index).text('Rule')
      $('.gridView'+index).hide()
      $('#listView'+index).show()

    } else {
      SetListStatus(!listStatus)
      $('#list-grid-exp'+index).text('User List')
      $('.gridView'+index).show()
      $('#listView'+index).hide()
    }
  }
    ...
    <>
      ...
      <div className="bg-white mt-1">
        ...
        <Transition.Root show={open} as={Fragment}>
          <Dialog as="div" className="relative sm:hidden" onClose={setOpen}>
            <Transition.Child
              as={Fragment}
            ...
            <div className="fixed inset-0 flex z-40">
              ...
              >
                <Dialog.Panel className="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-2 pb-12 flex flex-col overflow-y-auto">
...
{/* filter code end here */}
      ...
      {/* exp policy start here without filter panel*/}
      ...
      <div className='px-2'>
        ...
        <form className="divide-y divide-gray-200" id="allocateBudgets">
          {/* {arr2.length} */}
            ...
            <div key={index} >
              <div className="flex items-center justify-between gap-4">
               ...
               {/* for checkbox */}
                ...
                <div className="inc-dec-div sm:border sm:border-gray-300 p-2 pb-2 my-2 mx-auto exp-list-form">



                  {/* ============================= ALL INFORMATION  ============================== */}

                  <div className="border-b border-gray-200 pb-2 bg-slate-100">
                    <div className="flex flex-wrap items-center justify-between sm:flex-nowrap">
                      <div className="">
                        ...
                        {index == singleIndex ?
                          (deptName != "" ?
                            (<><div className="sm:grid mt-0 mb-4 sm:grid-cols-3 sm:items-start">
                              <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Dept name: {deptName}</p>
                              <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Expense Type: {expName}</p>
                              <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Total fund: {total}</p>
                              <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Invested fund: {invested}</p>
                              <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Remaining fund: {rem}</p>


                              {data.id != null ?
                                <>
                                  <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Total users under this level: {totalUsers}</p>
                                  <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Total amount of this policy will be: {totalUsers * singleAmount}</p>
                                </>
                                :
                                (<>
                                  <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">level Total users under this level: {totalUsers}</p>
                                  <p className="mt-1 max-w-2xl text-gray-500 text-xs font-medium">Total amount of this policy will be: {totalUsers * singleAmount}</p>
                                </>
                                )}
                      ...
                      <div className="flex-shrink-0">
                        <button
                          ...
                          id={"list-grid-exp" + index}
                          type="button"
                          className="relative inline-flex items-center rounded-md border border-transparent bg-teal-500 px-2 py-1 text-sm font-medium text-white shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2"
                        >
                          User List
                        </button>
                      </div>
                  ...
                  <div className={'gridView' + index}>
                    ...
                    <div className="lg:grid lg:grid-cols-6 lg:gap-2 mt-4" >
                      ...
                      <div className="mb-3">
                        ...
                        <input type="text" required={true} id={'unique_ids' + index} disabled={true}
                          readOnly={false}
                      ...
                      <div className="mb-3">
                        ...
                        <select
                          ...
                          onChange={async (event) => {
                            ...
                            SetSingleDept(event.target.value);
                            var type = "notbutton";
                            //api function call
                            DeptOnchageFunction(event.target.value, index, type)
                          }}
                          autoComplete="dept_id-name"
                          className="mx-auto disable-class max-w-lg block p-2 border border-gray-300 focus:ring-teal-500 focus:border-teal-500 w-full shadow-sm sm:max-w-xs sm:text-sm rounded-md"
                        >
                      ...
                      <div className="mb-3">
                        ...
                        <select
                          ...
                          onChange={async (event) => {
                            dynamicChangeInput(index, event)
                            //this is for remaining fund, all available limits api 
                            //under the perticular dept and expence type

                            //api function call for add and edit
                            { data.id != null ? getFundAndLevelClone(singedept, event.target.value) : getFundAndLevel(event.target.value) }

                          }}
                          autoComplete="exp_type_id-name"
                          className="mx-auto disable-class max-w-lg block p-2 border focus:ring-teal-500 focus:border-teal-500 w-full shadow-sm sm:max-w-xs sm:text-sm border-gray-300 rounded-md"
                        >
                          {singleIndex == index ?
                            <>
                      ...
                      <div className={"mb-3 hidden-input-exp" + index}>
                        <label htmlFor="" className='font-medium text-sm'>Level:</label>
                        <select
                          id={"level_name" + index}
                          name="level_name"
                          required={true}
                          ...
                          onChange={async (event) => {
                            dynamicChangeInput(index, event)
                            //api call to get total user under thid dept and level
                            getAllUserUnderADeptAndLevel(singedept, event.target.value)


                          }}
                          autoComplete="level_name-name"
                          className="mx-auto disable-class max-w-lg block p-2 border  focus:ring-teal-500 focus:border-teal-500 w-full shadow-sm sm:max-w-xs sm:text-sm border-gray-300 rounded-md"
                        >
                          {singleIndex == index ?
                            <>
                              <option value="">Select levels</option>
                              {
                                filterlevel.map((lvl) => {
                                  return (
                                    <option value={lvl.level_name} key={lvl.id} selected={lvl.level_name == data.level_name ? 'selected' : ''}>{lvl.level_name}</option>
                                  )
                                })
                              }
                            </>
                        ...
                        <h1 className='valid' id={"err_lvl" + index}></h1>
                      </div>
                      ...
                      {/* amount_per_user */}
                      <div className={"mb-3 hidden-input-exp" + index}>
                        <label htmlFor="" className='font-medium text-sm'>Amount Per User:</label>
                        <input type="number" required={true} id={'amount_per_user' + index} disabled={true}
                          name="amount_per_user" placeholder='amount per user'
                          value={data.amount_per_user} onChange={event => {
                            dynamicChangeInput(index, event);
                      ...
                      <div className={"mb-3 hidden-input-exp" + index}>
                        <label htmlFor="" className='font-medium text-sm'>Amount Per Transaction:</label>
                        <input type="number" required={true} id={'amount_per_transaction' + index} disabled={true}
                          name="amount_per_transaction" placeholder='amount per transaction'
                          value={data.amount_per_transaction} onChange={event => {
                            dynamicChangeInput(index, event);

                          }}
                          className="mx-auto disable-class max-w-lg p-2 block w-full shadow-sm border focus:ring-teal-500 focus:border-teal-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md"
                        />
                        <h1 className='valid' id={"err_amnt_trsns" + index}></h1>
                      </div>
                    ...
                    <span
                      ...
                      onClick={(e) => {
                        ...
                        var sAmount = value[index]['amount_per_user'];
                        var sAmountTransaction = value[index]['amount_per_transaction'];
                        ...
                        if (sLevel == "") {
                          $("#err_lvl" + index).text("enter level")
                          flag = 1;
                        } else {
                          $("#err_exp" + index).text("")
                          flag = 0;
                        ...
                        if (sAmount == "") {
                          $("#err_amnt" + index).text("enter user amount")
                          flag = 1;
                        } else {
                          $("#err_amnt" + index).text("")
                          flag = 0;
                        ...
                        if (sAmountTransaction == "") {
                          $("#err_amnt_trsns" + index).text("enter transaction amount")
                          flag = 1;
                        } else {
                          $("#err_amnt_trsns" + index).text("")
                          flag = 0;
                        }
                        ...
                        if (sUid != "" && sDept != "" && sExp != "" && sLevel != "" && sAmount != "" && sFrom != ""
                          && sto != "" && sPeriodicity != "" && sAmountTransaction != "") {
                    ...
                    {/* {data.id != null ? <><Link className='flex justify-end' to={"/budgets/expence-policy/userlist/" + data.id}>User list under this policy ({data.totalUserUnderPolicy})</Link></> : ''} */}
                  </div>

                  <div id={'listView'+index}
                   style={{display:'none'}}>
                    {/* hello {index} */}
                    <PolicyUserList id={data.id} listStatus={listStatus}/>
                  </div>
                </div>
             ...
             {/* EDIT AND DELETE BUTTON PART START */}
                ...
                {/* EDIT AND DELETE BUTTON PART END */}
                </div>
              </div>
            </div>
          ))}
