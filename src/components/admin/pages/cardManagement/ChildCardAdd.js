...
const ChildCardAdd = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (data.result.code == 200) {
      ...
      dispatch(setList(data.result.list)); // reset the list
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="sm:flex sm:items-center">
          <div className="sm:flex-auto">
            {/* <h1 className="text-2xl font-semibold text-gray-900">Add cards for {name}</h1> */}
          </div>
          <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <Link to={"/management#tab_cards"}
          ...
          <form className="space-y-8 divide-y divide-gray-200" id="cardMngAdd" onSubmit={handleSubmit}>
            <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
              ...
              <div className="pt-8 space-y-6 sm:pt-10 sm:space-y-5">
                <div>
                  <h3 className="text-lg leading-6 font-medium text-gray-900">Add Cards for {name}</h3>
                </div>
